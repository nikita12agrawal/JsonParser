...
public class parser {
	...
	{
		FileReader fr = new FileReader("Sample.json");
		int i;
		char c;
...
class Start{
	...
	public int matchArray(ArrayList<Character> jsonString,int index) throws ParseException
	{
		...
		if (jsonString.get(index) != ']') {
			throw new ParseException("Missing ] for array at location "+index);
		}
		/*if(jsonString.get(index)==']')
			index++;*/
		return index;
	...
	public int matchNumber(ArrayList<Character> jsonString,int index)
	{
